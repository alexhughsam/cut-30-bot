# Niche Map — Riftbound & the TCG Content World

*Built 2026-07-14 against the pre-registered EVIDENCE-STANDARD.md (including its §5
post-review raises). Freshness bar: no event inside the next 14 days missing; refresh
before any script rides an event. Review log: §9.*

**Scope declaration (per standard §5.6 and §6.5):** this map's **creator claims** are
scoped to **Western/English-language platforms only** (YouTube, Twitch, TikTok, X,
Instagram). Riftbound's largest market is plausibly China (Vendetta is the first
simultaneous EN+CN release) and **no Bilibili/Douyin/Weibo discovery was attempted** —
the CN creator hierarchy is a declared blind spot, not a certified absence. The
**calendar** is scoped **globally** (it includes CN, KR, EU, APAC events).

**Evidence grade cap:** the research environment's egress proxy policy-blocked every
direct platform page and stats tracker (§8). All metrics are **search-snippet grade
"(snippet)"**, retrieved 2026-07-14. Snippet evidence is admissible but weaker than a
fetch, and **no post in this map cleared the ≥3×-own-median "top post" bar** — so no
REMIX skeleton may be finalized from this map alone without first pulling the source
video's numbers by another route.

---

## 1. THE NEXT 14 DAYS (Jul 14–28) — the window that matters most

| Date | Game | Event | Source / confidence |
|---|---|---|---|
| **NOW → Jul 17** | Riftbound | **Vendetta preview season** — daily partner card reveals (Dev Drop Jul 6, 6pm PT) | [playriftbound.com](https://playriftbound.com/en-us/news/announcements/vendetta-preview-season/) + [SCG schedule](https://articles.starcitygames.com/riftbound/full-schedule-of-previews-for-riftbound-vendetta-revealed/) (snippet) — HIGH |
| **Jul 16, 6pm PT** | Riftbound | **Full ban list + rules update announcement** (beyond pre-announced Stealthy Pursuer). Biggest reaction moment of the fortnight — speed beats polish | [riftbound.gg](https://riftbound.gg/stealthy-pursuer-banned-full-ban-announcement-to-come-july-16-2026/) + [SCG](https://articles.starcitygames.com/riftbound/riftbound-announces-surprise-ban-to-address-new-infinite-combo-in-vendetta/) + [Escapist](https://www.escapistmagazine.com/news-riftbound-announces-a-proactive-ban-ahead-of-set-4-with-more-ban-updates-on-the-way/) (snippet) — HIGH |
| Jul 16 | Pokémon | Mega Evolution—Pitch Black releases **digitally on Pokémon TCG Live** (day before paper) | [pokemon.com July products](https://www.pokemon.com/us/news/check-out-every-pokemon-tcg-product-release-in-july-2026) (snippet) — MED |
| Jul 17 | Pokémon | Mega Evolution—Pitch Black paper release (Mega Darkrai ex chase) | [pokemon.com](https://www.pokemon.com/us/pokemon-news/the-pokemon-tcg-mega-evolution-pitch-black-expansion-arrives-july-17-2026) (snippet) — HIGH |
| Jul 17–19 | MTG | **MagicCon Amsterdam + Pro Tour Marvel Super Heroes.** Preview panels: **Reality Fracture Jul 17, The Hobbit debut Jul 18**; three new-set announcements expected over the weekend | [magic.gg](https://magic.gg/news/the-pro-tour-and-magic-spotlight-series-in-2026) + [SCG panels](https://articles.starcitygames.com/magic-the-gathering/magiccon-amsterdam-preview-panels-announced/) (snippet) — HIGH; three-sets item MED ([mtgrocks](https://mtgrocks.com/mtg-reality-fracture/)) |
| Jul 17 | Lorcana | Attack of the Vine! prerelease (game stores + select Disney parks/stores) + $99.99 Disney-store-only Curator's Collection Heroines Edition (scarcity story) | [disneylorcana.com](https://www.disneylorcana.com/en-US/news/2026/06/attack-of-the-vine-press-release) (snippet) — HIGH / [cultureslate](https://www.cultureslate.com/news/disney-lorcana-reprints-are-meant-to-address-scarcity) MED |
| **Jul 17 (probable)** | SWU | Set 8 Ashes of the Empire **wide release Jul 17**; the earlier Jul 10/11 date is almost certainly the **prerelease** (prerelease products launch one week before wide release — [skillshotzgaming prerelease guide](https://skillshotzgaming.com/star-wars-unlimited-ashes-of-the-empire-prerelease-guide/); wide release Jul 17 per [cbr](https://www.cbr.com/star-wars-ashes-of-empire-official-release-july-2026/) + [screenrant](https://screenrant.com/star-wars-ashes-of-empire-july-2026/)). Reconciliation per review pass 4; official page still 403 — re-verify before riding release day | MED-HIGH |
| **Jul 18(–19)** | One Piece | **Regional Championships Wolverhampton (UK, 1,024 players) + São Paulo (BR)** — the flagship circuit feeding Worlds | [organizedplay.events #170](https://tickets.organizedplay.events/Event/Index/170) + [Limitless](https://onepiece.limitlesstcg.com/tournaments/433) + [official Regionals page](https://en.onepiece-cardgame.com/events/regional-season1-26-27.html) (snippet) — HIGH |
| **Jul 19** | Lorcana | **Disney Lorcana Challenge Yokohama** (Pacifico Yokohama, capped 2,300 players; winner earns a Worlds 2026 invite — one of the season's largest Lorcana events) | [lorcanaplayer calendar](https://lorcanaplayer.com/lorcana-release-event-calendar/) + [Challenge Season 2 tracker](https://lorcanaplayer.com/disney-lorcana-challenge-season-2-tracker/) (snippet) — MED-HIGH |
| Jul 19 | One Piece | Treasure Cup Wolverhampton + Treasure Cup São Paulo — **companion/side events** to the Jul 18 Regionals | [Victory Road calendar](https://tcg.victoryroad.pro/onepiece-2026-calendar/) + [TCGplayer July events](https://www.tcgplayer.com/content/article/Upcoming-Events-for-the-One-Piece-Card-Game-July-2026/7d1f4af0-4c21-4c16-959e-a1b97e9e2914/) (snippet) — MED-HIGH |
| ~Jul 11–22 | Riftbound | July Summoner Skirmish store events (window conflict: [official events page](https://playriftbound.com/en-us/news/announcements/riftbounds-upcoming-official-events/) says "begins Jul 22," stores list Jul 4–18 events) | official events page + store pages (snippet) — MED, conflict flagged §7 |
| **Jul 23–26** | Riftbound | **San Diego Comic-Con presence** — official roadmap booth stop + a listed Vendetta Pre-Rift at Bards & Cards tied to SDCC | [bleedingcool roadmap](https://bleedingcool.com/games/teamfight-tactics-and-riftbound-culminate-at-convergence-fest/) + [sdccblog](https://sdccblog.com/events-default/riftbound-vendetta-pre-rift-event-at-bards-cards/) (snippet) — MED-HIGH |
| Jul 24 | Riftbound | **Stealthy Pursuer ban effective** (kills the Stealthy Pursuer + Eye of the Herald + Renata Glasc infinite enabled by Vendetta's Chaos+Order pairing; Guskin: team "isn't opposed to infinite combos by default") | [SCG](https://articles.starcitygames.com/riftbound/riftbound-announces-surprise-ban-to-address-new-infinite-combo-in-vendetta/) + [Guskin on X](https://x.com/davetron/status/2074533556706226295) (snippet) — HIGH |
| **Jul 24–30** | Riftbound | **Vendetta Pre-Rift (prerelease) at LGS worldwide** — sealed: 15-card mini-deck + 5 boosters + promo; first hands-on-cards window | [ICv2 62570](https://icv2.com/articles/sponsored/view/62570/sponsored-riftbounds-vendetta-arrives-july-31-new-champions-new-products-organized-play-support) + [UVS locator example](https://locator.riftbound.uvsgames.com/events/533931) (snippet) — HIGH |
| Jul 24 | Lorcana | Attack of the Vine! full release (Set 13, 207 cards) | [disneylorcana.com](https://www.disneylorcana.com/en-US/news/2026/06/attack-of-the-vine-press-release) (snippet) — HIGH |
| Jul 24 | Gundam | GD05 Freedom Ascension (1st-anniversary set) | [gundam-gcg.com](https://www.gundam-gcg.com/en/products/gd05.html) (snippet) — HIGH |
| **Jul 24–26** | MTG | **Magic Spotlight: Marvel Super Heroes — Brussels** ($60K prize pool; main event Jul 25–26) | [magic.gg](https://magic.gg/news/assemble-for-magic-spotlight-marvel-super-heroes-in-las-vegas-and-brussels) + organizer [fanfinity](https://www.fanfinity.gg/event/magic-spotlight-series-brussels-2026/) (snippet) — HIGH (publisher + organizer agree, per §7.1) |
| **Jul 25–26** | Lorcana | **China's first regional Championship — Shanghai Disneyland Hotel** (Challenge Season 2; winner qualifies for Worlds) | [lorcanaplayer tracker](https://lorcanaplayer.com/disney-lorcana-challenge-season-2-tracker/) + [Mushu Report](https://mushureport.com/china-announces-full-tournament-structure-road-to-worlds/) (snippet) — MED-HIGH |
| Jul 24–26 **or 25–27 (CONFLICT)** | SWU | **Galactic Championship, Las Vegas (Venetian)** — biggest SWU event of the year, new-set meta debut | [official](https://galacticchampionship.starwarsunlimited.com/2026/home) says 24–26 vs [SWU Competitive Hub](https://www.swu-competitivehub.com/galactic-championship-july-25-27-the-venitian-resort-las-vegas/) says 25–27 (snippet) — HIGH it happens, day conflict flagged §7 |
| **Jul 25** | One Piece | **Treasure Cup Peoria, IL (US)** | [Victory Road](https://tcg.victoryroad.pro/onepiece-2026-calendar/) + [TCGplayer](https://www.tcgplayer.com/content/article/Upcoming-Events-for-the-One-Piece-Card-Game-July-2026/7d1f4af0-4c21-4c16-959e-a1b97e9e2914/) (snippet) — MED-HIGH |
| ~~Jul 26~~ | Riftbound | ~~MSI Daejeon presence~~ — **PHANTOM, removed by review pass 3**: MSI 2026 ran Jun 28–Jul 12 ([Wikipedia](https://en.wikipedia.org/wiki/2026_Mid-Season_Invitational), [Liquipedia](https://liquipedia.net/leagueoflegends/Mid-Season_Invitational/2026)); Riftbound's booth was the Fan Fest demo Jul 3–12 ([Inven Global](https://x.com/InvenGlobal/status/2070328361193263309)). Riot's stale "Upcoming Official Events" promo page listed it — source-vs-organizer conflict, organizer wins | past event — do not ride |
| ~~Jul 26–28~~ | Riftbound | ~~EVO USA 2026~~ — **PHANTOM, removed by review pass 3**: Evo 2026 ran Jun 26–28, results published ([evo.gg](https://evo.gg/events/evo2026/), [EventHubs results](https://www.eventhubs.com/news/2026/jun/26/evo-2026-results/)). Same stale promo-page source | past event — do not ride |
| Mid-Jul | Riftbound | China's Unleashed Major, northern China — exact dates `[UNVERIFIED]`; Parthenay (France) roadmap stop ~mid-July `[UNVERIFIED]` | roadmap via [bleedingcool](https://bleedingcool.com/games/teamfight-tactics-and-riftbound-culminate-at-convergence-fest/) (snippet) |

## 2. THE REST OF THE 60 DAYS (Jul 29 – Sep 12)

**Riftbound:**
- **Jul 30–Aug 2 — Gen Con Indy**: learn-to-play, Vendetta prerelease events, 2v2 sealed, panel, "Ahri, Alluring" promo ([gencon.com/events/327242](https://www.gencon.com/events/327242) — HIGH).
- **Jul 31 — VENDETTA GLOBAL RELEASE**: first simultaneous EN+CN launch; 166 cards (one source "160+", conflict §7), 9 Legends (down from 12): Akali (poster), Zed, Shen, Kennen, Mel, Jayce, Ambessa, Renekton, Nasus; first new domain pairings incl. Chaos+Order; 50+ Showcase incl. Overnumbered. Products: Booster Display $119.99; first-ever Showdown Decks (Zed vs Shen, $34.99); Vendetta Vault (price `[UNVERIFIED]`). ([ICv2 62570](https://icv2.com/articles/sponsored/view/62570/sponsored-riftbounds-vendetta-arrives-july-31-new-champions-new-products-organized-play-support), [riftboundguide](https://riftboundguide.com/2026/06/07/riftbound-vendetta-everything-we-know/), [runeweave](https://runeweave.com/set/VEN) — HIGH date, MED count.)
- **Aug 21–23 — RQ Barcelona**, Fira de Barcelona — first RQ of the Vendetta season, top 64 qualify ([Eventbrite](https://www.eventbrite.com/e/riftbound-regional-qualifier-barcelona-tickets-1992319038877) — HIGH). **Sep 4–6 — RQ Singapore**, Singapore EXPO ([Eventbrite](https://www.eventbrite.com/e/riftbound-regional-qualifier-singapore-tickets-1992780327604) — HIGH). RQ Los Angeles Sep 25–27 just outside window (MED).
- Showdown Series (sanctioned third-party): Gatineau Aug 8, Auckland + Speyer Aug 15, Milwaukee Aug 22 — all `[UNVERIFIED — single aggregated summary]`.
- Aug 29–Sep 1 — Riftbound demos at Seattle Asmodee event (probably PAX West — venue name `[UNVERIFIED]`).
- **September — "Korea Enters the Rift": Korean-language Origins launches** (exact date `[UNVERIFIED]`) — a whole new language market opening inside the window ([official announcement](https://playriftbound.com/en-us/news/announcements/korea-enters-the-rift/) — HIGH that it's September, snippet).
- Ongoing: **Nexus Nights** weekly LGS play ([uvsgames.com](https://uvsgames.com/news/battle-you-on-the-rift-introduction-to-riftbound-organized-play/)); **Le Rift Tour** France, 16 LGS → finals at Paris Games Week Oct 22–25 ([Bienvenue France](https://riftbound.leagueoflegends.com/en-us/news/announcements/bienvenue-france/)).
- Planning horizon: Radiance previews Sep 28–Oct 9, release Oct 23, Ekko/Seraphine/Evelynn (MED); NA Regional Championship at Convergence Fest, Las Vegas Dec 11–13, $50K ([TFT announcement](https://teamfighttactics.leagueoflegends.com/en-us/news/announcements/celebrate-tft-and-riftbound-at-convergence-fest/) — HIGH; "first vs second NA RC" conflict §7); first Worlds 2027 (HIGH).

**Adjacent games:**
- **MTG**: everything funnels into **The Hobbit** — Amsterdam debut panel Jul 18, Gen Con wave Jul 30–Aug 2, prerelease week Aug 7–13 ([wpn.wizards.com](https://wpn.wizards.com/en/news/the-hobbit-dates-and-details)), **global release Aug 14**, Spotlights Brisbane Aug 28–30 + Dallas Sep 4–6 ([magic.gg](https://magic.gg/news/prepare-for-magic-spotlight-the-hobbit-in-brisbane-and-dallas)). **B&R announcement Aug 10** (HIGH, set in the [Jun 29 B&R](https://magic.wizards.com/en/news/announcements/banned-and-restricted-june-29-2026)). Reality Fracture previews expected from ~Sep 8 `[UNVERIFIED — projection]`. Live controversy: One Ring art plagiarism confirmed (Dan Frazier/Marta Nael — [draftsim](https://draftsim.com/mtg-the-hobbit/)), Standard-legal UB debate, no Commander precons.
- **Pokémon**: supply normalization + anti-scalping is THE story (2M+ unit reprint waves at MSRP; Nintendo ID-authentication from ~Aug `[UNVERIFIED date]` — [mynintendonews](https://mynintendonews.com/2026/07/04/nintendo-addresses-pokemon-tcg-scalping/)); 30th-anniversary cycle: Storm Emeralda (JP) Jul 31 ([pokebeach](https://www.pokebeach.com/2026/06/storm-emeralda-set-starring-mega-rayquaza-ex-officially-revealed)) → **Worlds San Francisco Aug 28–30** + first PokémonXP festival ([pokemon.com](https://www.pokemon.com/us/news/register-for-the-pokemonxp-and-2026-worlds-single-day-interest-list), [Moscone](https://www.moscone.com/events/pokemon-world-championships-2026)) → 30th Celebration set Sep 16, first same-day worldwide launch ([tcg.pokemon.com](https://tcg.pokemon.com/en-us/expansions/30th-celebration/)). New season starts Sep 1.
- **One Piece**: **OP-17 "The World's Strongest Warriors"** — 4th anniversary, first-ever global simultaneous release. **JP Aug 22; EN reported officially confirmed Aug 26** ([SNKRDUNK](https://snkrdunk.com/en/magazine/2026/07/13/opcg-one-piece-card-game-op-17-the-worlds-strongest-warriors-price-guide-top-chase-cards/), [tinpanblog](https://tinpanblog.com/onepiece-card-op17-worlds-strongest-warrior-august22-2026-guide-en/)); some US retailers list Aug 28 (conflict §7). Pre-release events from Aug 21 (MED). **Jul 31 EN release wave: Skypiea Starter Decks ST-31–36, Illustration Box Vol.7/8, Best Selection Vol.6** ([TCGplayer](https://www.tcgplayer.com/content/article/Upcoming-Events-for-the-One-Piece-Card-Game-July-2026/7d1f4af0-4c21-4c16-959e-a1b97e9e2914/) — MED). Regionals Season 2 begins Aug (per-city dates not retrieved). B&R update plausible Aug–Sep `[UNVERIFIED]`.
- **Lorcana**: the Challenge/Championship season is live NOW (Yokohama Jul 19 and Shanghai Jul 25–26 — see §1), then Championships at Disney resorts — NA at Disneyland Hotel Aug 28–30, EU at Disneyland Paris Sep 11–13 ([disneylorcana.com](https://www.disneylorcana.com/en-US/news/2026/03/disney-lorcana-challenge-championships-information) — HIGH); scarcity/exclusivity discourse.
- **Yu-Gi-Oh**: Worlds Tokyo Aug 29–30 ([konami.com](https://www.konami.com/yugioh/worldchampionship/2026/en/) — HIGH); Magnificent Monsters Sep 2–4 ([yugioh-card.com](https://www.yugioh-card.com/eu/product/magnificent-monsters/)); NA WCQ decklists peaking this week ([Konami blog](https://yugiohblog.konami.com)). Next F&L list unannounced `[UNVERIFIED]`.
- **SWU**: Ashes meta debuts at Galactic Championship Jul 24–26; Homeworlds moved up to October (MED, [screenrant](https://screenrant.com/star-wars-ashes-of-empire-july-2026/)).

## 3. RIFTBOUND SET & FORMAT FACTS (verified against handoff leads)

- Set history: Origins CN Aug 1 2025 / EN Oct 31 2025; Spiritforged CN Dec 12 2025 / EN Feb 13 2026; Unleashed CN Apr 10 2026 / EN May 8 2026; Vendetta global Jul 31 2026 (multiple secondaries incl. [SCG schedule](https://articles.starcitygames.com/riftbound/riftbound-2026-set-release-schedule/), snippet — HIGH). Handoff lead CONFIRMED and sharpened.
- Standard = Origins + 2 years; ~4 sets/year; first rotation with Set 10 in early 2028 ([Escapist/Guskin interview](https://www.escapistmagazine.com/news-riftbound-set-rotation-confirmed-via-dave-guskin-interview-with-rich-phu/) — HIGH). CONFIRMED.
- No digital client; Guskin: digital is "not a matter of if, but when and how" ([pcgamesn](https://www.pcgamesn.com/league-of-legends/riftbound-online-future-scarcity) — HIGH). CONFIRMED with nuance.
- Fan-project line: the canonical prohibition on automating rules/gameplay is Riot's **Digital Tools Policy** ([developer.riotgames.com/docs/riftbound](https://developer.riotgames.com/docs/riftbound), mid-2025), reiterated in the Feb 2026 State of the Game. Handoff attribution partially corrected (MED). We make content *about* the game — unaffected.

## 4. LIVE META & MARKET (as of 2026-07-14)

- **Meta**: end-of-Unleashed meta is **Master Yi, Wuju Bladesman #1 (~10% metashare)**, then Irelia, Blade Dancer and Diana, Scorn of the Moon; top domain pairs Calm/Chaos (~14%), Calm/Body, Order/Mind ([riftdecks.com/legends](https://riftdecks.com/legends) + [riftbound.gg/tier-list](https://riftbound.gg/tier-list/), July 2026 — MED-HIGH). **Factor (@KaChowFactor) won RQ Hartford Jun 19–21** (1,953 players, $25K) with Yi over Diana 2-0; Yi is the only legend with 3 RQ wins in Unleashed ([riftbound.gg](https://riftbound.gg/hartford-regional-qualifier/), [Riftbound Report](https://x.com/RiftboundReport/status/2068930642801279046), [riftdecks](https://x.com/riftdecks/status/2069018086401867909) — HIGH).
- **Market/supply**: handoff's "$90–100 chase singles" is **stale/understated at the top**: signed Overnumbered legends — Kai'Sa (Origins) ~$2,356; Jinx (Origins, signed) ~$972; LeBlanc (Unleashed, signed) ~$801; Diana ~$794 ([josephwriteranderson](https://www.josephwriteranderson.com/blog/the-most-expensive-riftbound-origins-cards) + pricecharting, snippet — MED; prices move). Origins displays fell **$337 → $183 (−45%) in 9 weeks** on reprint waves while weekly sales tripled (bangforyourbucktcg.com — `[UNVERIFIED — full URL not captured; single source]`). Riot's anti-scalping apparatus: reprints "until the market reaches more healthy quantities," purchase limits, Preorder Drawing lottery (ran Jul 6–8; results emails ~Jul 13 — [product-drawing FAQ](https://playriftbound.com/en-us/news/announcements/product-drawing-faq/)), free Flash promos to LGS that crushed a $72 panic spike ([thegamer](https://www.thegamer.com/riftbound-ruins-scalpers-days-releases-flash-free/) — MED-HIGH).
- **This week's discourse** (second-hand — Reddit/X bodies unreachable; inferred from indexed videos/posts, MED): (a) Stealthy Pursuer surprise ban + "how many more on Jul 16?"; (b) Vendetta preview reactions — Mel and infinite loops, Red Zed, Chaos+Order, 12→9 legends; (c) Preorder Drawing results / LGS allocation sizes (one store: "4 boxes"); (d) restock chatter.

## 5. RIFTBOUND CREATOR MAP (Western/English platforms only — see scope declaration)

*Per standard §5.1–5.3: rankings contain only accounts fully meeting the standard
(game-specific scale + ≥2 different-type corroborations naming them for Riftbound);
everyone else is a listed CANDIDATE with the exact failure named. All metrics
snippet-grade, 2026-07-14. Discovery routes per platform are listed at the end of this
section. No ranked Riftbound creator leaderboard exists anywhere except Twitch
trackers (HIGH).*

**Meets the standard: NO ACCOUNT, on any platform.** Review pass 2 examined the two
entries previously listed here and both failed the raised corroboration rule (§6.1):
every citation was self-sourced, machine-generated, or mis-cited. Per §5.3, an empty
list is the honest outcome. The strongest un-walked corroboration route is Riot's
official **[2026 Creator Activations program](https://playriftbound.com/en-us/news/announcements/2026-creator-activations/)**
(preview-partner continuity, creator badges at RQs, a Creator Discord, "creators
who've established themselves as authorities on Riftbound") — mining its partner
roster is the fastest way to promote candidates legitimately, and it was missed in
discovery (now logged).

**Candidates (standard not fully met — exact failure named; may NOT anchor a REMIX):**

- **LotharHS** (Twitch ~297K / X ~33.3K / YT @LotharHSRiftbound / riftmeta.net) — the closest to qualifying. (a) scale: [streamscharts](https://streamscharts.com/channels/lotharhs) ~297K followers, avg 555 viewers/30d — **conflict: his own X post self-reports 200–400 concurrents** ([X](https://x.com/LotharHS/status/2051635106557673480)); count is channel-wide, output is Riftbound-branded. FAILS §6.1: the "DRX Riftbound analyst" corroboration was **self-sourced (his bio) and misdescribed** — [Liquipedia](https://liquipedia.net/valorant/Lothar) (independent wiki, a valid naming act) describes him as a DRX **streamer** and **VCT VALORANT broadcast analyst** who is also a Riftbound content creator; riftbound.gg hosting his decks is passive ecosystem presence, not naming. Valid corroborations found so far: one (Liquipedia). Needs one more.
- **AliEldrazi** (Twitch ~54K, avg 38 viewers — [streamscharts](https://streamscharts.com/channels/alieldrazi) / X 16.8K / dedicated Riftbound YT, 11K `[figure uncertain]`) — genuinely Riftbound-dedicated (declared MWF cadence, [his X post](https://x.com/AliEldrazi/status/2005689171684569276) — self-sourced, fine for cadence, not corroboration). FAILS §6.1: prior Metafy citation was a **wrong URL** (pointed to eMOEtional; his is [metafy.gg/@alieldrazi/shop](https://metafy.gg/@alieldrazi/shop)); the [topdeck.gg item](https://topdeck.gg/deck/saturday-10000-riftbound-leage-of-legends-tcg-tournament/KlQkTkILKgWzjUunxxMtxMLY9Y62) is a participation decklist, not naming; riftdecks feature same defect; sponsorships self-reported. Valid corroborations: zero pinned. 20+ MTG Pro Tours pedigree real but not Riftbound-naming.

- **imls** (Twitch 642K, avg 1,121 viewers/30d — [twitchtracker](https://twitchtracker.com/imls), [twitchstats](https://twitchstats.net/streamer/imls)) — FAILS §5.1: scale is channel-wide and **LoL-dominated** (SullyGnome: most-streamed game last 30d is League of Legends; "mainly streams League of Legends but sometimes streams Riftbound" — [sullygnome](https://sullygnome.com/channel/imls)); FAILS §5.2: corroborations are two stats trackers (same type, and trackers ≠ corroboration). Still likely the biggest single audience that regularly sees Riftbound streams.
- **Tolarian Community College** (YT 1M subs, [milestone video](https://www.youtube.com/watch?v=BldZtCD6vw4)) — FAILS §5.2 for Riftbound specifically: no third party found naming it a notable *Riftbound* creator; audience built on MTG. Its ["How To Play Riftbound"](https://www.youtube.com/watch?v=sPvBt2mroDY) showed 70.9K views (~Nov 2025) `[UNVERIFIED as overperformer — no median]`.
- **JohnPanio** (Twitch 1.2M — [followercharts](https://www.followercharts.com/games/twitch/riftbound)) — FAILS §5.1/5.2: likely large variety channel; activity data conflicts ("not active last 30 days" vs a Jul 10 Riftbound VOD, 7h46m, 12.3K views).
- **Challenger TCG** (YT, subs unknown) — FAILS §1(a): no scale evidence found at all. Strongest official credit among dedicated channels: **official Riot Vendetta spoiler partner** ([reveal video](https://www.youtube.com/watch?v=X5XmxVaquYY)) + Metafy "largest competitive community" + interviews.
- **eMOEtional** (YT/X ~5.5K, subs unknown) — FAILS §1(a): no scale evidence. (b) is strong: [Metafy](https://metafy.gg/@emoetional), SCGCON Vegas 10K Top-32 ([Piltover Archive](https://piltoverarchive.com/decks/view/0233dddc-bfd7-456d-ab3e-7a5d733d8d4e)). Notable post: Miracle Draven "first tier 0 deck" call ([X](https://x.com/MreMOEtional/status/2024700020885533044)) `[UNVERIFIED engagement]`.
- **GrappLr** (Twitch 100K — [twitchstats](https://twitchstats.net/streamer/grapplr)) — FAILS §5.2: one corroboration.
- **Snnuy** (Twitch 49.8K avg 97 viewers — [streamscharts](https://streamscharts.com/channels/snnuy); YT 121K — [Let's Play Index](https://www.letsplayindex.com/channels/267671-snnuy)) — FAILS §5.2: corroborations not independently established.
- **Riftbound Report / Marshall_K** (X 21–26.7K — [piclur](https://piclur.com/profile/RiftboundReport); Twitch 1.1K) — the niche's news desk; FAILS §5.2 as documented (ecosystem citations not pinned to two named different-type sources).
- **jfwong** (TikTok 52.8K, avg ~29K views/video — [urlebird](https://urlebird.com/user/jfwong/); hosted Riot's /ALLChat — [self-sourced X post](https://x.com/jfwong/status/1992259864492667015)) — FAILS §5.2: corroboration one-source/self-sourced. **The only account anywhere in this map with a usable per-video baseline.**
- Also discovered, insufficient data to classify: CovertGoBlue (official Origins reveal partner — [X](https://x.com/covertgoblue/status/1935741364832211008)), Silent Knight (official Vendetta spoiler partner — [YT](https://www.youtube.com/watch?v=rsm2eRkEpoc)), RUNEBOYS (ES), Runes & Rift, Riftlab, DUNC, @kzcollectibles (TikTok), LotharHS Riftbound (YT).

**Platform verdicts (per §6.2, both dimensions shown):** *By documentable
Riftbound-standing*, LotharHS and AliEldrazi lead but neither yet clears the raised
bar. *By raw audience regularly seeing Riftbound content*, the biggest accounts are
imls (642K, LoL-dominant) and JohnPanio (1.2M, variety) — larger than every
Riftbound-native account, and neither is a Riftbound creator in the standing sense.
Neither list alone answers "top creator"; read them together. YouTube: no account
fully meets the standard; TikTok: effectively unclaimed by any verifiable
Riftbound-native creator; Instagram: official @playriftbound (52K) dominates; X:
LotharHS and Riftbound Report lead what's visible.

**Discovery routes used (per standard §5.5):** YouTube — YT search snippets, community
references, aggregator mentions, preview-partner coverage. Twitch — tracker sites
(followercharts/twitchmetrics/streamscharts/twitchstats/sullygnome snippets), Twitch
directory search results, creator cross-references. X — profile snippets, mirror sites
(piclur, instalker), press references. TikTok — discover-page snippets, creator
cross-refs, urlebird mirror. Instagram — IG search snippets, linktree, official news.
(48+ queries total; every discovered account is listed above or in the "also
discovered" line — none withheld.) Note: "no ranked Riftbound creator leaderboard
exists anywhere except Twitch trackers" is downgraded to **MED** per review pass 4 —
a universal negative can't be HIGH at snippet grade with riftbound.zone/en/creator
unread. **Route missed in passes 1–2, now logged:** Riot's
official creator-program pages ([2026 Creator Activations](https://playriftbound.com/en-us/news/announcements/2026-creator-activations/))
— the single most authoritative corroboration source; not yet mined (page body 403,
roster extraction pending).

## 6. FORMAT RAID MAP (from the wider TCG creator economy)

*Evidence grade: reconstructed from titles, series descriptions, and press — no
caption/timestamp-level verification was possible (§8). Raid priority for Riftbound
short-form; saturation checked against Riftbound-specific searches.*

1. **Market/price-call personality** — OPEN. Sites saturated ([riftbound.gg/prices](https://riftbound.gg/prices), riftboundstats), but no video personality owns the documented arc: bot-swept preorders ([Dexerto on X](https://x.com/Dexerto/status/2011406450724319566)) → Origins −45% crash `[UNVERIFIED — see §4]` → Riot-vs-scalpers. Model: Alpha Investments (102K subs; [Kotaku-corroborated](https://kotaku.com) market-mover `[article URL not captured]`).
2. **Banlist live-reaction ritual** — breakdowns exist (same-day videos on the March bans), but the **recurring ritual persona** (Cimoooooooo's formula: emotional-verdict titles, every list, same format; 370K subs, ran 2019–2025 — [Kotaku obituary](https://kotaku.com/yugioh-alex-cimo-cimoooooooo-youtube-cancer-death-2000702971)) is unclaimed in Riftbound. Next trigger: **Jul 16, 6pm PT.** (Alex Cimo died 2026-06-03 — treat the lineage respectfully.)
3. **Progression series** — fully UNCLAIMED ("Riftbound progression series" surfaced nothing). Only 4 sets exist; catch-up-from-Origins is cheap and timed. Model: Cimo/Nyhmnim serialized box-per-set format ([yugioh.fandom](https://yugioh.fandom.com/wiki/Alex_Cimo)).
4. **Chase-pull shorts with dollar-value punchline** — genre-proven ([CardDeckr](https://carddeckr.com): god-pack videos "rack up millions of views"; Whatnot card economy ~$6–8B GMV, sources differ §7); no dominant Riftbound face; RiftTube sample shows most Riftbound pack openings get single-digit–1K views (MED). Open, not dead.
5. **Daily spoiler speed-reaction in preview windows** — partner slots press-claimed (Wargamer, Card Gamer, Escapist, Challenger TCG, Silent Knight, CovertGoBlue); short-form daily speed-reaction unclaimed. Vendetta window closes **Jul 17**; next: Radiance, Sep 28.
6. **High-stakes "I bought X" stunt** — UNCLAIMED in Riftbound. Press-verified genre ceiling: Logan Paul's $3.5M fake-box video, #1 YouTube trending ([Vice](https://www.vice.com), [Newsweek](https://www.newsweek.com) `[article URLs not captured — headline claims only]`). Riftbound variant sitting there: "I bought a case at peak vs now" against the crash arc.
7. **Grading reveals** — UNCLAIMED for Riftbound; PSA's claimed Pokémon-grading halt `[UNVERIFIED — TikTok-snippet only]`.

**AVOID (saturated):** tier lists ([riftbound.gg](https://riftbound.gg/tier-list/), [mobalytics](https://mobalytics.gg/riftbound/tier-list), [riftdecks](https://riftdecks.com), riftmeta + multiple YT channels); generic deck techs (exception: budget-constraint angle, unfound in Riftbound). **Weak fit:** collection nostalgia (game <1 year old in the West).

## 7. UNRESOLVED CONFLICTS (reported, not silently resolved)

1. SWU Ashes of the Empire release: RECONCILED by review pass 4 — Jul 10/11 was the prerelease, **Jul 17 wide release (probable)**; official page still unfetched, re-verify before riding release day.
2. One Piece OP-17 EN day: Aug 26 (officially confirmed per SNKRDUNK) vs Aug 28 (US retailers); JP Aug 22.
3. Riftbound July Summoner Skirmish window: "begins Jul 22" vs store events Jul 15–18.
4. March 2026 Riftbound ban wave: "4 cards + 3 battlefields effective Mar 31" vs "7 cards, April" (likely same event, 7 total — MED).
5. Convergence Fest: "first" vs "second" NA Regional Championship.
6. Vendetta card count 166 vs "160+"; reveal-season start: official Jul 6–17 vs one site's "Jun 22" (likely teaser-window language; Mel/Jayce pre-teased on MSI broadcast Jun 29).
7. Whatnot GMV: ~$8B (2024) vs $6B forecast (2025) — different years/methods.
8. JohnPanio: 1.2M followers vs "not active in last 30 days" vs Jul 10 Riftbound VOD.
9. LotharHS viewership: tracker "avg 555" vs self-reported 200–400 concurrents.
10. SWU Galactic Championship: Jul 24–26 (official site) vs Jul 25–27 Venetian (SWU Competitive Hub).
11. Riot's "Upcoming Official Events" promo page vs organizers' own calendars: the promo page listed MSI and EVO appearances that organizer dates show already ended (see §1 phantom rows) — treat that page as MED-grade until cross-checked, per standard §7.

## 8. WHAT WE COULDN'T SEE (lockout transparency, per the standard)

Every direct fetch of platform pages and trackers was policy-blocked by the research
environment's egress proxy (403 at CONNECT), 2026-07-14: youtube.com, tiktok.com,
instagram.com, x.com, reddit.com, socialblade.com, twitchtracker/twitchmetrics/
streamscharts/followercharts/sullygnome, urlebird, web.archive.org (no cache fallback),
plus playriftbound.com, riftbound.gg, riftbound.zone/en/creator (the one dedicated
creator directory — biggest single evidence loss), icv2.com, mobalytics.gg,
starcitygames.com, dotesports.com, magic.wizards.com, pokemon.com news,
en.onepiece-cardgame.com restriction pages. (Reviewer 1 independently confirmed the
lockout.)

**Also not attempted (distinct from blocked — confessed per review):**
(1) **Chinese platforms** (Bilibili/Douyin/Weibo) — zero discovery for the game's
possibly-largest market; (2) starwarsunlimited.com's official product page for the
Set 8 date conflict; (3) One Piece per-city Regionals Season 2 dates.

**Consequences:** zero platform-native metrics (all snippet-grade); no per-account
median baselines → **no post qualifies as a verified overperformer**, so no REMIX
skeleton can be finalized from this map alone; this-week discourse is second-hand;
eMOEtional's and Challenger TCG's subscriber counts never found; TikTok/Instagram
rankings incomplete; format skeletons in §6 are title/press-grade reconstructions.

**Possible fixes (for the operator's $20/month budget):** a YouTube Data API key (free
tier likely sufficient) or a paid social-stats service would convert most
`[UNVERIFIED]` metrics to platform-native evidence and make the ≥3×-median top-post
bar computable. Flagging once, per the budget rule.

## 9. REVIEW LOG

- **Built 2026-07-14** from four parallel research agents (Riftbound state/calendar,
  Riftbound creators, adjacent-TCG calendar, format economy), gathering against
  EVIDENCE-STANDARD.md as pre-registered.
- **Review pass 1 (fresh adversarial reviewer): FAIL.** Nine ranked gaps + seven
  attacks on the standard itself. Reviewer's spot-checks confirmed 15+ load-bearing
  claims (Jul 16 announcement, Jul 24 ban, Vendetta Jul 31, Hartford result, Worlds
  SF, Hobbit Aug 14, Cimo, Gen Con) with URLs now folded in. Gaps closed in this
  revision: One Piece Treasure Cups + Jul 31 product wave added; full URLs attached
  (claims without a capturable URL downgraded to `[UNVERIFIED]`); Riftbound SDCC
  Jul 23–26 added; TCC tagged CANDIDATE; imls retagged CANDIDATE (LoL-dominant
  channel-wide scale; tracker-only corroboration); SWU conflict reweighted toward
  Jul 11; MTG Amsterdam row corrected (Reality Fracture Jul 17 / Hobbit Jul 18 / PT
  Marvel Super Heroes); China blindness declared as scope limitation; LotharHS
  viewer conflict + OP-17 Aug 26/JP Aug 22 updated. Standard raised (§5.1–5.6):
  game-specific scale, enforced different-type corroboration, no CANDIDATEs in
  rankings, full-URL rule, documented discovery routes, declared geographic scope.
- **Review pass 2 (second fresh reviewer, blind to pass 1): FAIL.** Six gaps + six
  standard attacks. Spot-checks confirmed the core Riftbound spine (Jul 16
  announcement, Jul 24 ban + Pre-Rift, Jul 31 release, SDCC) as "accurate and
  script-safe." Gaps closed in this revision: Riftbound at MSI Daejeon Jul 26 + EVO
  USA Jul 26–28 added (from Riot's own events page, missed in pass 1); **the
  "Meets the standard" creator list emptied** — LotharHS demoted (DRX corroboration
  was self-sourced and misdescribed; Liquipedia says DRX streamer / VCT VALORANT
  analyst) and AliEldrazi demoted (Metafy citation was a wrong URL; topdeck item is a
  participation record, not naming); Riot's 2026 Creator Activations program logged
  as the missed discovery route; One Piece Jul 18 Regionals added and Treasure Cups
  reframed as side events; Korea Origins launch (September) added; Pokémon TCG Live
  Jul 16 digital release added; calendar scope declared global. Standard raised
  (§6.1–6.5): "naming" defined (editorial/community acts only, no self-sourcing, no
  machine records, mis-cited = ✘), verified-standing must be shown alongside
  top-by-audience, ≥1 non-snippet/official source per ranked entry, publisher events
  index must be swept, calendar scope declared.
- **Review pass 3 (third fresh reviewer, blind to passes 1–2): FAIL.** Two real gaps,
  both **introduced by review pass 2's own gap-closing**: the "Riftbound at MSI
  Daejeon Jul 26" and "EVO USA Jul 26–28" rows were phantoms — Riot's "Upcoming
  Official Events" promo page was stale; MSI ran Jun 28–Jul 12 (Riftbound booth =
  Fan Fest demo Jul 3–12) and Evo ran Jun 26–28. Reviewer confirmed the rest of the
  window ("compliance on A is otherwise solid; the failure is factual") and ruled the
  empty creator list "justified, not nihilism." Closed in this revision: both phantom
  rows struck (kept visibly as warnings), SWU Galactic Championship day conflict
  flagged (24–26 vs 25–27), conflict #11 added for the stale promo page. Standard
  raised (§7.1–7.3): organizer's own dates mandatory for third-party events,
  "upcoming" pages presumed stale until validated, review-pass additions face the
  same verification as original claims.
- **Review pass 4 (fourth fresh reviewer, blind to passes 1–3): FAIL** — but confirmed
  the entire Riftbound spine, the phantom-striking, the SDCC row, both One Piece
  Regionals, the creator section ("no creator claim stated above its evidence"), and
  independently vindicated the §7.2 stale-page rule (it caught three more
  month-shifted phantom leads and cleared them itself). Three gaps, all adjacent-game
  window completeness, closed in this revision: **MTG Spotlight Marvel Super Heroes
  Brussels Jul 24–26** added ($60K, publisher+organizer agree); **Lorcana Challenge
  Yokohama Jul 19** (2,300 players) and **China's first Championship, Shanghai
  Disneyland Hotel Jul 25–26** added; **SWU date lean inverted and reconciled** —
  Jul 10/11 was the prerelease, Jul 17 the probable wide release. Also downgraded the
  "no leaderboard" universal negative to MED. Standard raised (§8.1–8.3): index sweep
  extends to every covered game, every named game carries the completeness
  obligation, universal negatives cap at MED on snippets.
- **Review pass 5 (FINAL — cap):** pending. Per the loop rule, whatever pass 5 finds,
  the operator sees the survivor plus the last standing objection and makes the call.
